...
namespace SHIZ {
	...
	void MainServer::closeServer() {
		...
		for (QTcpSocket* follower : activeFollowers) {
			...
			if (follower->state() != QAbstractSocket::UnconnectedState) {
				follower->waitForDisconnected();
			}
			follower->deleteLater();
		}
		activeClients.clear();

		logger->log("All clients and followers disconnected and server stopped.");
	}
	...
	void MainServer::handleFollowerDisconnected() {
		QTcpSocket* followerSocket = qobject_cast<QTcpSocket*>(sender());
		if (followerSocket) {
			activeFollowers.removeAll(followerSocket);
			followerSocket->deleteLater();
			logger->log("Follower disconnected.");
		}
	}
